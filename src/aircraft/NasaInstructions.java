...
public class NasaInstructions {
    ...
    TableLand transformStringForCoordinateTableLand(String word) {
    ...
    private void instructionForNasa(TableLand tableLand, Sensor sensor) {
        for (int i = 0; i < aircrafts.size(); i++) {
            ...
            ControlAirship nasa = new Nasa(aircrafts.get(i), tableLand, sensor);
            Integer latestPosition = 0;
            ...
            if (i == 0) {
                ...
                if (i > 1) {
                    for (int j = 0; j < i; j++) {
                        latestPosition += subList.get(j);
                    }
                    finish = latestPosition + sizeInstruction;
                    nasa.moveAircraft(getCommands().subList(latestPosition, finish));
                } else {
                    finish = start + sizeInstruction;
    ...
    void positionAircraft(String word) {
    ...
    void coordinateAircraft(String word) {
    ...
    List<Integer> getTableLandCoordinates() {
